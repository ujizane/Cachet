...
class SqliteRepository implements MetricInterface
{
    ...
    public function getPointsForDayInWeek(Metric $metric, $day)
    {
        ...
        $value = 0;
        $query = DB::select("select {$queryType} as value FROM metrics JOIN metric_points ON metric_points.metric_id = metrics.id WHERE metrics.id = :metricId AND created_at > date('now', '-7 day') AND strftime('%Y%m%d', metric_points.created_at) = :timeInterval GROUP BY strftime('%Y%m%d', metric_points.created_at)", [
            'metricId'     => $metric->id,
